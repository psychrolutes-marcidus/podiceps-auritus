...
CREATE OR REPLACE VIEW draught_by_ship_type AS
...
CREATE OR REPLACE VIEW draught_by_all AS
    (SELECT DISTINCT draught,
                     count(*) AS samples
     FROM ais_data
     WHERE draught IS NOT NULL
     GROUP BY draught);
...
CREATE OR REPLACE VIEW draught_by_mmsi AS
